...
EGID=$(curl -k -s -H "Accept: application/json" -H "Content-Type: application/json" -H "Authorization: Token $DOJOKEY" --url "${DOJOURL}/api/v2/engagements/?limit=1000" | jq -c '[.results[] | select(.product == ${PRODID} and .engagement_type == "CI/CD")][0] | .id')
